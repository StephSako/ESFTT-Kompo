            ...
            $builder->add('idJoueur' . $j, EntityType::class, [
                ...
                'query_builder' => function (CompetiteurRepository $cr) use ($options, $builder) {
                    ...
                    for ($i = 0; $i < $options['nbMaxJoueurs']; $i++) {
                        ...
                        $request->andWhere('c.idCompetiteur NOT IN (SELECT IF(p' . $i . '.idJoueur' . $i . ' IS NOT NULL, p' . $i . '.idJoueur' . $i . ', 0) FROM App\Entity\RencontreDepartementale p' . $i . ' WHERE p' . $i . '.idJournee = d.idJournee AND p' . $i . '.idEquipe <> :idEquipe)');
                    }
