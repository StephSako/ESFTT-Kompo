        ...
        for($j = 1; $j <= $builder->getData()->getIdEquipe()->getIdDivision()->getNbJoueursChampDepartementale(); $j++) {
            $builder->add('idJoueur' . $j, EntityType::class, [
                ...
                'query_builder' => function (CompetiteurRepository $cr) use ($builder) {
                    ...
                    $str = '';
                    for ($i = 1; $i <= $builder->getData()->getIdEquipe()->getIdDivision()->getNbJoueursChampDepartementale(); $i++) {
                        $str .= 'p.idJoueur' . $i . ' = c.idCompetiteur';
                        if ($i < $builder->getData()->getIdEquipe()->getIdDivision()->getNbJoueursChampDepartementale()) $str .= ' OR ';
                    ...
                    $request->andWhere('(SELECT COUNT(p.id) FROM App\Entity\RencontreDepartementale p WHERE (' .$str . ') AND p.idJournee < :idJournee AND p.idEquipe < :idEquipe) < 2')
