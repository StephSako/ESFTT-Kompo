                    ...
                    if (in_array($nbEquipe, array_merge($equipesIDsCommon, $equipesToCreateIDs))) {
                        ...
                        if (in_array($nbEquipe, $equipesIDsCommon)) {
                            ...
                            usort($rencontresEquipeKompo, function ($element1, $element2) {
                                $datetime1 = ($element1->getDateRencontre())->getTimeStamp();
                                $datetime2 = ($element2->getDateRencontre())->getTimeStamp();
                        ...
                        foreach ($rencontresFFTT as $i => $rencontre) {
                            ...
                            $rencontreTemp['datePrevue'] = $rencontre->getDatePrevue();
                            ...
                            if ($i <= $championnatActif->getNbJournees() - 1) {
                                if (in_array($nbEquipe, $equipesIDsCommon)) {
                                    ...
                                    if (($isExempt != $rencontresEquipeKompo[$i]->isExempt()) ||
                                        (!$isExempt &&
                                            ($rencontresEquipeKompo[$i]->getAdversaire() != $rencontreTemp['adversaireFFTT'] ||
                                                ...
                                                $rencontreTemp['datePrevue'] != $rencontresEquipeKompo[$i]->getDateRencontre() ||
                                        ...
                                        if ($idClubAdversaire && !$adressesClubs[$idClubAdversaire]) {
                                            ...
                                            } catch (ClubNotFoundException|InvalidURIParametersException|NoFFTTResponseException|URIPartNotValidException $e) {
                                            }
                                        }
                                        ...
                                        $rencontreTemp['forceUpdateInfoAddresse'] = $rencontresEquipeKompo[$i]->getAdversaire() != $rencontreTemp['adversaireFFTT'];
                                    ...
                                    $rencontreToCreate
                                        ...
                                        ->setVilleHost(false)
                                        ->setConsigne(null)
                                        ->setDateRencontre($rencontresKompo[$i]->getDateRencontre())
                                        ->setReporte(false)
        ...
        if ($request->request->get('resetPlayers')) {
            ...
            if (count($championnatSearch) == 1) {
                ...
                if ($request->request->get('preRentreeResetChampionnats')) {
                    ...
                    //TODO A FAIRE POUR LES RENCONTRES
...
//                        $this->em->flush();
                    ...
                    foreach ($allChampionnatsReset[$championnat->getNom()]["preRentree"]["rencontres"] as $rencontreKompo) {
                        $rencontreKompo
                            ...
                            ->setReporte(false)
                            ->setDateRencontre($rencontreKompo->getDateRencontre())
                            ->setAdversaire(null);
                    }
                    $this->em->flush();
                    ...
                    foreach ($allChampionnatsReset[$championnat->getNom()]["matches"]["issued"] as $rencontresParEquipe) {
                        if ($rencontresParEquipe['enAttenteNouvellePhase']) {
                            ...
                            $rencontresParEquipe['rencontre']
                                ...
                                ->setReporte(false)
                                ->setLastUpdate(null)
                                ->setDateRencontre($rencontresParEquipe['datePrevue']);
                            ...
                            $rencontresParEquipe['rencontre']
                                ...
                                ->setDateRencontre($journeeToSet->getDateJournee());
