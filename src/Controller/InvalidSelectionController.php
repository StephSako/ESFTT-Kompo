...
class InvalidSelectionController extends AbstractController
{
    ...
    public function checkInvalidSelection($type, $compo, int $idJoueur, int $nbJournees, int $nbJoueurs){
        if ($idJoueur != null && $compo->getIdJournee()->getIdJournee() < $nbJournees) {
            ...
            else if ($type === 'paris') $this->deleteInvalidSelectedPlayers($this->rencontreParisRepository->getSelectedWhenBurnt($idJoueur, $compo->getIdJournee()->getIdJournee(), $compo->getIdEquipe()->getNumero(), $this->getParameter('limite_brulage_paris') - 1, $nbJoueurs), $nbJoueurs);
        }
    }
