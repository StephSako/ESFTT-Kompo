...
function sending(idCompetiteur){
    $('#loaderCertif' + idCompetiteur).show();
    $('#btnCertif' + idCompetiteur).hide();
}

function endSending(response, idCompetiteur, isFromAdmin, prenom){
    if (!response.status) {
        ...
        else $('.label-renew-certif' + idCompetiteur).removeClass('red').addClass('green');
        $('.label-renew-certif' + idCompetiteur).html(!isFromAdmin ? 'Renouvellement pour la rentrée<br><b>' + response.message + '</b>' : response.message);
    }
