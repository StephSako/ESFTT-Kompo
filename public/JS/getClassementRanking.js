function getClassementPoule(lienDivision, numeroEquipe) {
    if (!alreadyCalledClassementPoule.includes(numeroEquipe)) {
        ...
        $.ajax({
            ...
            dataType : 'json',
            success : (responseTemplate) => { templatingClassementPoule(numeroEquipe, responseTemplate); },
            error : () => { templatingClassementPoule(numeroEquipe, "<p style='margin-top: 10px' class='pastille reset red'>Le service de la FFTT rencontre des perturbations. Réessayez plus tard</p>"); }
        });
    }
}
